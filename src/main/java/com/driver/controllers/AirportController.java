...
public class AirportController {
    ...
    public double getShortestDurationOfPossibleBetweenTwoCities(@RequestParam("fromCity") City fromCity, @RequestParam("toCity")City toCity){
        ...
        for(Flight flight:flightDb.values()){
            if((flight.getFromCity().equals(fromCity))&&(flight.getToCity().equals(toCity))){ // i am comparing if two cities are equal  to my desired  locations cities
...
//        return count;
    ...
    public int calculateFlightFare(@RequestParam("flightId")Integer flightId){
        ...
        int totalNoOfBookings = flightToPassengerDb.get(flightId).size();
        int fare =  totalNoOfBookings*50 + 3000;
        return fare;
    }
